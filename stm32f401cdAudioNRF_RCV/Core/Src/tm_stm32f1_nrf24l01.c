...
inline uint8_t SPI_nrf_rx_tx(uint8_t value){	//basic funcion to transmit commands and get STATUS contents in return
	uint8_t contents;
	//SPI_FLAG_BSY
	//while( (NRF24L01_SPI->Instance->SR) & (SPI_SR_BSY) );
	while(( (NRF24L01_SPI->Instance->SR) & (SPI_FLAG_TXE))!= SPI_FLAG_TXE);
	NRF24L01_SPI->Instance->DR = value;


	while(( (NRF24L01_SPI->Instance->SR) & (SPI_FLAG_RXNE))!= SPI_FLAG_RXNE);
	contents = NRF24L01_SPI->Instance->DR;

	return contents;
}

uint8_t TM_SPI_Send(SPI_HandleTypeDef* thspi1,uint8_t data)
{
	uint8_t  dummy = 0;
	HAL_SPI_TransmitReceive(thspi1,&data,&dummy,1,0xffff);
	return dummy;
}
void TM_SPI_SendMulti(SPI_HandleTypeDef* thspi1, uint8_t *dataO, uint8_t * dataI, int count)
{
	HAL_SPI_TransmitReceive(thspi1,dataO,dataI,count,0xffff);
}
void TM_SPI_ReadMulti(SPI_HandleTypeDef* thspi1,uint8_t* data,uint8_t dummy,int count)
{
	int k;
	for(k=0;k<count;k++)
	{
	...
	HAL_SPI_TransmitReceive(thspi1,data,data,count,0xffff);
}

void TM_SPI_WriteMulti(SPI_HandleTypeDef* thspi1,uint8_t* data, int count)
{
	HAL_SPI_Transmit(thspi1,data,count,0xffff);
}
...
uint8_t TM_NRF24L01_Init(uint8_t channel, uint8_t payload_size) {
	...
	/* Config register */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_CONFIG, NRF24L01_CONFIG|(1 << NRF24L01_PWR_UP));
	HAL_Delay(100);
	uint8_t res = TM_NRF24L01_ReadRegister(NRF24L01_REG_CONFIG);
	printf("res=%x\n",res);
#ifdef NO_ACK
	...
	//
	TM_NRF24L01_WriteRegister(NRF24L01_REG_FEATURE, 	NRF24L01_REG_DEFAULT_VAL_FEATURE | 1);

	/* Enable RX addresses */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_EN_RXADDR, 0x1);

	/* Auto retransmit delay: 1000 (4x250) us and Up to 15 retransmit trials */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_SETUP_RETR, 0x0);
#else
	/* Enable auto-acknowledgment for all pipes */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_EN_AA, 0x3);

	//
	TM_NRF24L01_WriteRegister(NRF24L01_REG_FEATURE, 	NRF24L01_REG_DEFAULT_VAL_FEATURE );

	/* Enable RX addresses */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_EN_RXADDR, 0x3);

	/* Auto retransmit delay: 1000 (4x250) us and Up to 15 retransmit trials */
	TM_NRF24L01_WriteRegister(NRF24L01_REG_SETUP_RETR, 0x11);
#endif
...
uint8_t TM_NRF24L01_ReadRXBytes() {
	//uint8_t value;
	uint8_t dta[2] ;
	NRF24L01_CSN_LOW;
	//SPI_nrf_rx_tx(NRF24L01_READ_REGISTER_MASK(reg));
	//value = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	dta[0] = SPI_nrf_rx_tx(NRF24L01_R_RX_PL_WID_MASK);
	dta[1] = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	//dta[0] = NRF24L01_READ_REGISTER_MASK(reg);
	//dta[1] = NRF24L01_NOP_MASK;
	//TM_SPI_SendMulti(NRF24L01_SPI,dta,dta,2);
...
uint8_t TM_NRF24L01_ReadRegister(uint8_t reg) {
	...
	//SPI_nrf_rx_tx(NRF24L01_READ_REGISTER_MASK(reg));
	//value = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	dta[0] = SPI_nrf_rx_tx(NRF24L01_READ_REGISTER_MASK(reg));
	dta[1] = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	//dta[0] = NRF24L01_READ_REGISTER_MASK(reg);
	//dta[1] = NRF24L01_NOP_MASK;
	//TM_SPI_SendMulti(NRF24L01_SPI,dta,dta,2);
...
void TM_NRF24L01_ReadRegisterMulti(uint8_t reg, uint8_t* data, uint8_t count) {
	...
	{
		data[k] = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	}
...
void TM_NRF24L01_WriteRegister(uint8_t reg, uint8_t value) {
	NRF24L01_CSN_LOW;
	uint8_t dta[2] ;
	dta[0] = SPI_nrf_rx_tx(NRF24L01_WRITE_REGISTER_MASK(reg));
	dta[1] = SPI_nrf_rx_tx(value);
	//dta[0] = NRF24L01_WRITE_REGISTER_MASK(reg);
	//dta[1] = value;
	//TM_SPI_SendMulti(NRF24L01_SPI,dta,dta,2);
...
void TM_NRF24L01_WriteRegisterMulti(uint8_t reg, uint8_t *data, uint8_t count) {
	NRF24L01_CSN_LOW;
	SPI_nrf_rx_tx(NRF24L01_WRITE_REGISTER_MASK(reg));
	//TM_SPI_Send(NRF24L01_SPI, NRF24L01_WRITE_REGISTER_MASK(reg));
...
void  TM_NRF24L01_ClearInt()
{
//	NRF24L01_CE_LOW;
	NRF24L01_CLEAR_INTERRUPTS;
//	NRF24L01_CE_HIGH;
}
void TM_NRF24L01_FlushTx()
{
	NRF24L01_FLUSH_TX;
}
void TM_NRF24L01_PowerUpRxCont(void)
{
	/* Disable RX/TX mode */
	//NRF24L01_CE_LOW;
	/* Clear RX buffer */
...
void TM_NRF24L01_Transmit(uint8_t *data) {
	...
	SPI_nrf_rx_tx(NRF24L01_W_TX_PAYLOAD_MASK);
	for(int k=0;k<count;k++)
	{
		SPI_nrf_rx_tx(data[k]);
	}
...
void TM_NRF24L01_Transmit_NoACK(uint8_t *data) {
	...
	NRF24L01_CSN_LOW;
	/* Send write payload command */
	//TM_SPI_Send(NRF24L01_SPI, NRF24L01_W_TX_PAYLOAD_NOACK_MASK);
	/* Fill payload with data*/
	//TM_SPI_WriteMulti(NRF24L01_SPI, data, count);

	SPI_nrf_rx_tx(NRF24L01_W_TX_PAYLOAD_NOACK_MASK);
	for(int k=0;k<count;k++)
	{
		SPI_nrf_rx_tx(data[k]);
	}
...
void TM_NRF24L01_Transmit_NoACK1(uint8_t *data) {
	uint8_t count = TM_NRF24L01_Struct.PayloadSize;
...
void TM_NRF24L01_GetData(uint8_t* data) {
	...
	SPI_nrf_rx_tx(NRF24L01_R_RX_PAYLOAD_MASK);
	for(int k=0;k<TM_NRF24L01_Struct.PayloadSize;k++)
	{
		data[k] = SPI_nrf_rx_tx(NRF24L01_NOP_MASK);
	}
...
void TM_NRF24L01_SoftwareReset(void) {
	...
	TM_NRF24L01_WriteRegister(NRF24L01_REG_SETUP_AW, 	1); NRF24L01_REG_DEFAULT_VAL_SETUP_AW;
