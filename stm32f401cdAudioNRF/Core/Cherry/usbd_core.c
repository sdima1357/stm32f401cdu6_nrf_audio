...
static bool usbd_set_configuration(uint8_t config_index, uint8_t alt_setting)
{
   ...
   // printf("configure endpoints for this configuration/altsetting %d %d !!!!!!!!!!!\n",config_index,alt_setting);
    while (p[DESC_bLength] != 0U) {
        switch (p[DESC_bDescriptorType]) {
            case USB_DESCRIPTOR_TYPE_CONFIGURATION:
            	...
            	//printf("USB_DESCRIPTOR_TYPE_CONFIGURATION %d !!!!!!!!!!!!\n",cur_config);
            ...
            case USB_DESCRIPTOR_TYPE_INTERFACE:
            	...
            	//printf("USB_DESCRIPTOR_TYPE_INTERFACE %d !!!!!!!!!!!!\n",cur_alt_setting);
                break;

            case USB_DESCRIPTOR_TYPE_ENDPOINT:
               // printf("USB_DESCRIPTOR_TYPE_ENDPOINT %d %d %d %d!!!!!!!!!!!!\n",cur_config,cur_alt_setting,config_index,alt_setting);
   ...
   // printf("end configure endpoints for this configuration/altsetting %d %d !!!!!!!!!!!\n",config_index,alt_setting);
    return found;
}
    ...
    while (p[DESC_bLength] != 0U) {
        switch (p[DESC_bDescriptorType]) {
            ...
            case USB_DESCRIPTOR_TYPE_ENDPOINT:
                ...
                USB_LOG_DBG("ep_desc %d iface %u alt_setting %u\n",ep_desc->bEndpointAddress, iface, alt_setting);
                if (cur_iface == iface)
                {
                	ret= true;
                	if(1)//flag_open==0
                	{
						...
						if (cur_alt_setting != alt_setting)
						{
							...
							//??USB_EPStopXfer(ep_desc->bEndpointAddress);
                                           ...
                                           uint8_t **data, uint32_t *len)
{
    ...
    switch (setup->bRequest) {
        ...
        case USB_REQUEST_GET_DESCRIPTOR:
            USB_LOG_INFO("usbd_std_interface_req_handler!!%x %d %d \n",type,intf_num,g_usbd_core.intf_offset);
...
void usbd_add_interface(struct usbd_interface *intf)
{
    ...
    printf("usbd_add_interface %d %p %d\n",intf->intf_num,g_usbd_core.intf[g_usbd_core.intf_offset],g_usbd_core.intf_offset);
