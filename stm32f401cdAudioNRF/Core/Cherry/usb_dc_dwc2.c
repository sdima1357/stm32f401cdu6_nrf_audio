...
#define CONFIG_USB_DWC2_RX_FIFO_SIZE (512+128+64)
#endif
...
#else
#ifndef CONFIG_USB_DWC2_RX_FIFO_SIZE
#define CONFIG_USB_DWC2_RX_FIFO_SIZE (576*2+16)
#endif
...
#ifndef CONFIG_USB_DWC2_TX1_FIFO_SIZE
#define CONFIG_USB_DWC2_TX1_FIFO_SIZE (32)
#endif

#ifndef CONFIG_USB_DWC2_TX2_FIFO_SIZE
#define CONFIG_USB_DWC2_TX2_FIFO_SIZE (0)
#endif

#ifndef CONFIG_USB_DWC2_TX3_FIFO_SIZE
#define CONFIG_USB_DWC2_TX3_FIFO_SIZE (0)
#endif

#ifndef CONFIG_USB_DWC2_TX4_FIFO_SIZE
#define CONFIG_USB_DWC2_TX4_FIFO_SIZE (0)
#endif
...
#if 1
        USB_OTG_DEV->DEACHMSK |= USB_OTG_DAINTMSK_OEPM & (uint32_t)(1UL << (16 + ep_idx));
        USB_OTG_OUTEP(ep_idx)->DOEPINT = 0xFBFFU;
...
int USB_EPStopXferL(const uint8_t ep)
{
  __IO uint32_t count = 0U;
  uint8_t ep_idx = USB_EP_GET_IDX(ep);
  int  ret = 0;
  //printf("stop xfer  ep %d \n",ep);
  ...
  else /* OUT endpoint */
  {
    if (((USB_OTG_OUTEP(ep_idx)->DOEPCTL) & USB_OTG_DOEPCTL_EPENA) == USB_OTG_DOEPCTL_EPENA)
    {
      ...
      } while (((USB_OTG_OUTEP(ep_idx)->DOEPCTL) & USB_OTG_DOEPCTL_EPENA) ==  USB_OTG_DOEPCTL_EPENA);
    }
  }
...
int usbd_ep_close(const uint8_t ep)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep)) {
        if (USB_OTG_OUTEP(ep_idx)->DOEPCTL & USB_OTG_DOEPCTL_EPENA) {
           ...
           USB_OTG_OUTEP(ep_idx)->DOEPINT |= USB_OTG_DIEPCTL_SNAK;
        }
